...
def extract_count_subject(query: str) -> str:
    ...
    # 先頭の年月日・季節パターンを除去（日付・季節は parse_date_filter で別途処理）
    # 例: 「2024年春の初沈の故障」→「初沈の故障」。季節語を残すと subject が
    # 「春の初沈」になり 0 件マッチになるため、年に続く季節語も併せて除去する。
    q = re.sub(
        r'^\d{4}年(?:\d{1,2}月(?:\d{1,2}日)?)?(?:春|夏|秋|冬)?(?:から|以降|以前|まで|頃)?の?',
        '', query,
    ).strip()
    patterns = [
...
def is_specific_failure_count_query(query: str) -> bool:
    ...
    count_kws = [
        ...
        # 「〇〇の故障記録はありますか」のような存在問い合わせも、件数で答えるのが
        # 適切なため count として扱う（該当件数を返す＝「はい、N件あります」）。
        "記録はあり", "記録があり", "記録はない", "記録はなかった",
    ]
